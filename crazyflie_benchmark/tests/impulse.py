...
class ImpulseTest(TestStrategy):
    ...
    Applies a brief impulse input on the specified channel (roll, pitch, yaw rate, or thrust)
    ...
    def _execute_test(self) -> bool:
        ...
        if self.channel == "roll":
            ...
            # Yaw channel uses yaw rate in deg/s
            yaw_rate_val = float(self.amplitude)
        ...
        logger.info(
            f"Applying impulse: Roll: {roll}°, Pitch: {pitch}°, "
        ...
        while time.time() - start_time < self.duration:
            ...
            if not self.check_safety():
                logger.warning(
                    f"Impulse test on {self.channel} interrupted due to safety concerns!"
                )
        ...
        if test_successful:
            ...
            while time.time() - recovery_start_time < self.recovery_duration:
                ...
                if not self.check_safety():
                    logger.warning(
                        f"Recovery phase interrupted due to safety concerns!"
                    )
                    test_successful = False
                    break
